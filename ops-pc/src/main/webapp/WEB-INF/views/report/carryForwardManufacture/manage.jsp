...
<div class="titleCustom">
    ...
    <div id="inputer">
        <form id="form" class="form1">
			...
			<p id="paction" style="margin-left:55px"><a href="javascript:;" id="addSub" class="btn-blue2 btn-s2">增加会计分录</a></> <a id="deleteSub" class="btn-blue2 btn-s2">删除会计分录</a> <a id="deleteAll" class="btn-blue2 btn-s2">全部删除</a></p>
			<br/>
			...
			<p style="margin-left:130px"><a href="javascript:;" id="save" class="btn-yellow2 btn-xs">结转</a></p>
         </form>
    </div>
    <div id="list">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
    	...
    	var outSubjectCombo = $('#outSubjectName').fool('dhxComboGrid',{
            ...
            toolsBar:{
    	  	...
    	  	onSelectionChange:function(){
            	...
            	$("#outSubjectName").attr("subjectType",($('#outSubjectName').next())[0].comboObj.getSelectedText().type);
            }
    	});
    	...
    	/* $("#inSubjectName").fool('subjectCombobox',{
    		...
    		onClickIcon:function(){
            	$(this).combobox('hidePanel');
            	inSubjectChooser();
            	$("#inSubjectName").combobox('textbox').blur();
    		},
            onSelect:function(record){
            	$("#inSubjectId").val(record.fid);
            	$("#inSubjectName").attr("subjectType",record.type);
            }
    	}); */
    	...
    	/* $("#voucherWordId").fool('voucherWordTree',{
    		...
    		onLoadSuccess:function(node, data){
    			var childs=data[0].children;
    			for(var i=0;i<childs.length;i++){
    				if(childs[i].text=="记 "){
    					$("#voucherWordId").combotree("setValue",childs[i].id);
    					break;
    				}
    			}
    		},
...
,            },
    ...
    function outSubjectChooser(target){
    	chooserWindow=$.fool.window({'title':"选择转出科目",href:'${ctx}/fiscalSubject/window?okCallBack=selectOutSubject&singleSelect=false&onDblClick=selectOutSubjectDBC&onlyLeafCheck=true&flag=1',
    		onLoad:function(){($('#outSubjectName').next())[0].comboObj.closeAll();}});//解决IE点击弹窗下拉框不消失问题
    }
    function selectOutSubject(rowData){
    	var fids="";
    	var names="";
    	for(var i=0;i<rowData.length;i++){
    		console.log(rowData[i].fid);
    		fids+=rowData[i].fid+",";
    		names+=rowData[i].name+",";
        	if(rowData[i].type!=6){
        		$("#outSubjectName").attr("subjectType",rowData[i].type);
        	}
    	}
    ...
    function inSubjectChooser(target){
    	chooserWindow=$.fool.window({'title':"选择转入科目",href:'${ctx}/fiscalSubject/window?okCallBack=selectInSubject&onDblClick=selectInSubjectDBC&singleSelect=true&flag=1',
    		onLoad:function(){inSubjectCombo.closeAll();}});//解决IE点击弹窗下拉框不消失问题
    }
    	...
    	if($("#outSubjectName").next().form("validate")&&$("#inSubjectName").next().form("validate")){
        	...
        	/* if($("#outSubjectName").attr("subjectType")!=6||$("#inSubjectName").attr("subjectType")!=6){
        		if($("#outSubjectName").attr("subjectType")!=6){
        			$.fool.alert({time:2000,msg:"转出科目包含非损益科目！",fn:function(){}});
        		}else{
        			$.fool.alert({time:2000,msg:"转入科目不是损益科目！",fn:function(){}});
        		}
        	} */
        	var arry=outSubjectIds.split(",");
        	for(var i=0;i<arry.length;i++){
        		if(arry[i]==inSubjectId){
        			$.fool.alert({time:1000,msg:"转出科目与转入科目不能重复。"});
        			return false;
        		}
        	}
        	var obj={
    ...
    $("#deleteSub").click(function(){
        ...
        for(var i=0;i<ids.length;i++){
            $.ajax({
                type : 'post',
                url : '${ctx}/manufacture/delete',
                data : {fid:getFid(ids[i])},
                dataType : 'json',
                async:false,
                success : function(data) {
                    if(data.returnCode != '0'){
                        $.fool.alert({time:2000,msg:data.message,fn:function(){
                            $('#subjectList').trigger("reloadGrid");
                        }});
                        return false;
                    }
    ...
    $("#save").click(function(){
    	...
    	if($("#voucherWordId").next().form("validate")&&$("#fiscalPeriodId").next().form("validate")){
    		...
    		$.ajax({
    			type : 'post',
    			url : '${ctx}/manufacture/makeVoucher',
    			data : {voucherWordId:voucherWordId,fiscalPeriodId:fiscalPeriodId,voucherDate:voucherDate,resume:voucherResume},
    			dataType : 'json',
    			success : function(data) {
    				if(data.returnCode == '0'){
    					if(data.data==" "){
    						...
    						$.fool.alert({msg:'操作成功！凭证字号为：'+data.data,fn:function(){
    							$('#subjectList').trigger("reloadGrid"); 
        					}});
    					}
    				}else{
    					if(data.errorCode=="101"){
    						$.fool.confirm({title:'确认',msg:data.message,fn:function(r){
        						if(r){
        							$.ajax({
        								type : 'post',
        			   					url : '${ctx}/manufacture/makeVoucher?flag=1',
        			   					data : {voucherWordId:voucherWordId,fiscalPeriodId:fiscalPeriodId,voucherDate:voucherDate,resume:voucherResume},
        			   					dataType : 'json',
        			   					success : function(data) {
        			   						if(data.returnCode == '0'){
