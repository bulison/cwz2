...
<body>
          <div class="form1" >
              <form id="form">
                <input name="type" id="type" type="hidden" value="5"/>
				...
				<p><font><em>*</em>科目：</font><input id="subjectName" show="false" name="subjectName" type="text" class="textBox" value="${bankBill.subjectName}"/></p>
...
/* $("#resume").combotree({
	...
	onLoadSuccess:function(node, data){
		...
		$("#resume").combotree('setText',"${bankBill.resume}");
	},
	onBeforeSelect:function(node){
		if(!node.id){
			return false;
...
function selectMemberDBC(rowData){
 ...
 <%--$("#subjectName").click(function(){--%>
	<%--chooserWindow=$.fool.window({'title':"选择科目",href:'${ctx}/fiscalSubject/window?okCallBack=selectSubject&onDblClick=selectSubjectDBC&singleSelect=true&cashSubject=1&flag=1'});--%>
<%--});--%>
