...
<c:when test="${param.billCode eq 'sctl'}">退料单</c:when>
<c:when test="${param.billCode eq 'cptk'}">成品退库</c:when>
</c:choose>
</h1>
</td></tr>
...
<c:if test="${param.billCode eq 'cgrk'}">
<tr><td colspan="100" style="text-align:left;">
注：货物验收无误后，需签名或盖章确认，三十日内无异常反馈，视同客户确认接收认可。
</td></tr>
</c:if>
<tr>
<td style="text-align:left;border-bottom:0px solid #000;padding-top:15px;" colspan="100">
<c:choose>
<c:when test="${param.billCode eq 'cprk'}">
<div style="float:right;width:300px;">仓管：</div>
入库人：
</td>
</c:when>
<c:when test="${param.billCode eq 'sctl'}">
<div style="float:right;width:300px;">仓管：</div>
退料人：
</td>
</c:when>
<c:when test="${param.billCode eq 'cptk'}">
<div style="float:right;width:300px;">仓管：</div>
退库人：
</td>
</c:when>
<c:when test="${param.billCode eq 'scll'}">
<div style="float:right;width:300px;">仓管：</div>
领料人：
</td>
</c:when>
<c:when test="${param.billCode eq 'pdd'}">
盘点人签名：
</c:when>
<c:otherwise>
<!-- <div style="float:right;width:300px;">收货单位及经手人(盖章)：</div> -->

<div style="text-align: right;float: right;">当前页：<span tdata="PageNO">#</span>&nbsp;&nbsp;/&nbsp;&nbsp;<span tdata="PageCount">#</span></div>
经手人：
</td>
</c:otherwise>
</c:choose>
...
if("undefined" == typeof texts||"undefined" == typeof thead||"undefined" == typeof vals){
	...
	var _str = "",_val="&nbsp;&nbsp;",_align='left',_width = 100/3;
	//文本区域
	_str = "<tr>";
	for (var i = 0; i < texts.length; i++) {
		...
		}else if(texts[i].colspanVal != undefined){
			_val = (texts[i].key.length==0) ? "&nbsp;&nbsp;" : vals[texts[i].key];
			_str+="<td style='text-align:"+_align+"' class='title'>"+texts[i].title+"</td><td class='val' colspan='"+texts[i].colspanVal+"'>"+_val+"</td>";
		}else{
			_val = (texts[i].key.length==0) ? "&nbsp;&nbsp;" : vals[texts[i].key];
		...
		if("undefined" != typeof tfoot){
			...
			for(var i=0;i<tfoot.length;i++){
				...
				if(tfoot[i].dtype==0){//当页统计
					_str += "<td><span tdata='SubCount' tindex='"+_ind+"'>"+tfoot[i].text+"</span></td>";
				}else if(tfoot[i].dtype==1){//统计全部
					_str += "<td><span tdata='AllCount' tindex='"+_ind+"'>"+tfoot[i].text+"</span></td>";
				}else if(tfoot[i].dtype==2){//当前页金额
					_str += "<td><span tdata='SubSum' tindex='"+_ind+"' format='0.00'>"+tfoot[i].text+"</span></td>";
				}else if(tfoot[i].dtype==3){//当前页金额大写
					_str += "<td><span tdata='SubSum' tindex='"+_ind+"' format='UpperMoney'>"+tfoot[i].text+"</span></td>";
				}else if(tfoot[i].dtype==4){//所有记录总金额
					_str += "<td><span tdata='AllSum' tindex='"+_ind+"' format='0.00'>"+tfoot[i].text+"</span></td>";
				}else if(tfoot[i].dtype==5){//所有记录总金额大写
					_str += "<td><span tdata='AllSum' tindex='"+_ind+"' format='UpperMoney'>"+tfoot[i].text+"</span></td>";
				}else{
					_str += "<td>&nbsp;</td>";
				} 
			}
