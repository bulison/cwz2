...
<c:set var="billCode" value="xsfp" scope="page"></c:set>
<c:set var="billCodeName" value="销售发票单" scope="page"></c:set>
...
.input_div{display: none;background:#F5F5F5; padding: 10px 0px 5px 0px; border: 1px solid #D5DBEA;position: absolute;right: 20px; top:91px;z-index: 1;}
...
<div class="titleCustom">
 ...
 <div id="addBox"></div> 
	...
	<input type=checkbox value='1' name="recordStatus">已审核<input type=checkbox value='2' name="recordStatus">已作废</p>
			...
			<p>银行账户：<input id="bankName" _class="bankName-combogrid" class="textBox"/><input type="hidden" name="bankId" id="bankId" /></p> -->
...
$('#dataTable').jqGrid({
	...
	colModel:[
	  		...
	  		{name:'code',label:'单号',align:"center",width:100,sortable:true,<fool:tagOpt optCode="xsfpAction1">formatter:codeLinkNew</fool:tagOpt>},
			...
			{name:'customerName',align:"center",label:'客户',width:100},
			...
			<fool:tagOpt optCode="xsfpAction">
	  		{name:'action',align:"center",label:'操作',formatter:actionFormatNew}
	  		</fool:tagOpt>
	      ],
	      gridComplete:function(){
	    	  	<fool:tagOpt optCode="xsfpAction2">//</fool:tagOpt>$('.btn-del').remove();
				<fool:tagOpt optCode="xsfpAction3">//</fool:tagOpt>$('.btn-copy').remove();
				<fool:tagOpt optCode="xsfpAction4">//</fool:tagOpt>$('.btn-approve').remove();
				<fool:tagOpt optCode="xsfpAction5">//</fool:tagOpt>$('.btn-cancel').remove();
	    	  warehouseAll();
	      }
