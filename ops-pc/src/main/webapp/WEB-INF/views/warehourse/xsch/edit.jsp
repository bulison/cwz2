...
<body>
          ...
          <div class="form1" >
              <form id="form">
				...
				<p><font>合计金额：</font><input id="totalAmount" class="textBox" readonly="readonly" value=""/></p>
				<p><font>免单金额：</font><input id="freeAmount" class="textBox" readonly="readonly" value="${obj.freeAmount}"/></p>
				<p style="width:75.8%;"><font>备注：</font><input data-options="validType:'maxLength[200]'" id="describe" class="textBox" style="width:80%;" value="${obj.describe}"/></p><br/>
...
$(function(){
	...
	if('${obj.recordStatus}'==null||'${obj.recordStatus}'==''){
		...
		$("#btnBox").append('<p><input type="button" id="save" class="btn-blue2 btn-xs" value="保存" /></p> <fool:tagOpt optCode="xschAction3"><p><input type="button" id="copy" class="btn-blue2 btn-xs" value="复制" /></p> </fool:tagOpt> <p><input type="button" id="print" class="btn-blue2 btn-xs" value="打印" /></p> <p><input type="button" id="refreshForm" class="btn-blue2 btn-xs" value="刷新"/></p>');
		...
		$("#btnBox").append('<fool:tagOpt optCode="xschAction3"><p><input type="button" id="copy" class="btn-blue2 btn-xs" value="复制" /></p> </fool:tagOpt> <fool:tagOpt optCode="xschAction5"><input type="button" id="cancel" class="btn-blue2 btn-xs" value="作废" /></p> </fool:tagOpt> <p><input type="button" id="print" class="btn-blue2 btn-xs" value="打印" /></p> <p><input type="button" id="refreshForm" class="btn-blue2 btn-xs" value="刷新"/></p>');
		...
		$("#form").find("input").attr('disabled','disabled');
		$("#addGoods").attr('disabled','disabled');
		$("#btnBox").append('<fool:tagOpt optCode="xschAction3"><p><input type="button" id="copy" class="btn-blue2 btn-xs" value="复制" /></p> </fool:tagOpt> <p><input type="button" id="print" class="btn-blue2 btn-xs" value="打印" /></p>');
	}
		...
		$("#goodsList").datagrid({
			...
			columns:[[
						...
						{field:'inWareHouseName',title:'仓库',width:100,editor:{type:'combotree',options:{url:"${ctx}/basedata/warehourseList",required:true,onLoadSuccess:function(node,data){
							if(data[0].id!=""){
								var node=$(this).tree("find",data[0].id);
								$(this).tree('update',{
									target: node.target,
					    ...
					    {field:'unitId',hidden:true,sortable:true,width:100},
				  		{field:'unitName',title:'单位',formatter:function(value){
							...
							onSelect:function(record){
							    ...
							    $.post("${ctx}/goods/getOtherPrice",{billType:'xsch',fid:goodsId,unitId:record.fid,goodsSpecId:goodsSpecId,customerId:$("#customerId").val()},function(data){lowestPriceField.text(data.lowestPrice);unitPriceField.numberbox('setValue',data.referencePrice);});
							    $(this).combobox('validate');
							}}},sortable:true,width:100},
						{field:'scale',title:'换算关系',sortable:true,width:100},
						{field:'quentity',title:'数量',editor:{type:'numberbox',options:{required:true,precision:2,validType:'positive'}},sortable:true,width:100},
...
$("#customerName").click(function(){
	chooserWindow=$.fool.window({'title':"选择客户",width:780,height:470,href:'${ctx}/customer/window?okCallBack=selectCus&onDblClick=selectCusDBC&singleSelect=true'}); 
});

$("#inUserName").click(function(){
	chooserWindow=$.fool.window({'title':"选择业务员",width:780,height:470,href:'${ctx}/member/window?okCallBack=selectUser&onDblClick=selectUserDBC&singleSelect=true'});
});
...
$("#relationName").click(function(){
	chooserWindow=$.fool.window({
		'title':"选择销售订单",
		href:'${ctx}/salebill/xsdd/window?okCallBack=selectRelation&onDblClick=selectRelationDBC&singleSelect=true&billCode=xsdd&cusId='+$("#customerId").val(),
		onClose:function(){
			if($('#salewin').html()!=''){
...
$("#addGoods").click(function(){
	chooserWindow=$.fool.window({'title':"选择货品",width:1074,height:500,href:'${ctx}/goods/window?okCallBack=selectGoods&onDblClick=selectGoodsDBC&singleSelect=false&billType=xsch&customerId='+$("#customerId").val()});
});
	...
	$('#form').form('enableValidation'); 
		if($('#form').form('validate')){
			...
			$('#save').attr("disabled","disabled");
			    $.post('${ctx}/saleOutStock/xsch/save',{details:JSON.stringify(details),fid:id,updateTime:updateTime,voucherCode:voucherCode,code:code,billDate:billDate,customerId:customerId,deptId:deptId,inMemberId:inMemberId,endDate:endDate,describe:describe,totalAmount:totalAmount,inWareHouseId:inWareHouseId,relationId:relationId,customerPhone:customerPhone},function(data){
...
$('#copy').click(function(e) {
	var fid=$("#fid").val();
	$('#addBox').window("refresh","${ctx}/saleOutStock/xsch/edit?id="+fid+"&mark=1");
...
function saverow(target){
	var lowestPrice=$(target).parents("[field='action']").siblings("[field='lowestPrice']").children().text();
...
function selectCus(rowData){
	...
	if(!$("#inUserId").val()){
		$("#inUserId").val(rowData[0].memberId);
		$("#inUserName").val(rowData[0].memberName);
	}
	$("#relationName").val("");
	$("#relationId").val("");
	chooserWindow.window('close');
...
function selectCusDBC(rowData){
	...
	if(!$("#inUserId").val()){
		$("#inUserId").val(rowData.memberId);
		$("#inUserName").val(rowData.memberName);
	}
	$("#relationName").val("");
	$("#relationId").val("");
	chooserWindow.window('close');
...
function selectRelation(rowData){
	...
	$("#depName").combotree('setValue',rowData[0].deptId)
	$("#inUserName").val(rowData[0].inMemberName);
	$("#inUserId").val(rowData[0].inMemberId);
...
function selectRelationDBC(rowData){
	...
	$("#depName").combotree('setValue',rowData.deptId)
	$("#inUserName").val(rowData.inMemberName);
	$("#inUserId").val(rowData.inMemberId);
