...
<%-- <c:set var="billName" value="${param.billCode=='bsd'?'报损单':param.billCode=='cgdd'?
							...
							'销售订单':param.billCode=='xsth'?'销售退货':''}" scope="page"></c:s --%>et>
...
<form action="" class="bill-form myform" id="warehousebillForm">
	...
	<div id="bill" class="shadow" style="margin-top:50px;">
		...
		<div class="in-box" id="list2">
			...
			<p>
			    <font>仓库：</font><input _imp="true" name="inWareHouseName" _class="warehouse" id="inWareHouseName" value="${isDetail ? obj.inWareHouseName : obj.inWareHouseId}" class="textBox" type="text" data-options="{required:true,novalidate:true}"/>
			    <font>盘点员：</font><input _imp="true" id="inMemberName" name="inMemberName" _class="member-combogrid" value="${obj.inMemberName}" class="memberBox textBox" type="text"/>
			</p>
			<p>
...
<c:otherwise>
initEdit('${param.flag}','${obj.recordStatus}','pdd','盘点单',${obj.details});
</c:otherwise>
</c:choose>
</script>
</body>
