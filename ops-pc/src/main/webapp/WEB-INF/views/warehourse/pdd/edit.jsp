...
<body>
          ...
          <div class="form1" >
              <form id="form">
				...
				<p><font><em>*</em>仓库名称：</font><input id="inWareHouseId"  class="textBox"/></p>
				<p><font><em>*</em>盘点员：</font><input id="inMemberName" name="inMemberName" value="${obj.inMemberName}" class="textBox"/></p>
				<p style="width:65%;"><font>备注：</font><input data-options="validType:'maxLength[200]'" name="describe" id="describe" class="textBox" style="width:80%;" value="${obj.describe}"/></p><br/> 
...
$(function(){	
	...
	textBox($("#search-userCode"));
	textBox($("#search-userName"));
	textBox($("#search-userPhone"));
	...
	if('${obj.recordStatus}'==null||'${obj.recordStatus}'==''){
		$("#btnBox").append(' <fool:tagOpt optCode="pddAdd"><p><input type="button" id="save" class="btn-blue2 btn-xs" value="保存" /></p></fool:tagOpt>');
	}else if('${obj.recordStatus}'==0){
		$("#btnBox").append('<fool:tagOpt optCode="pddAction1"><p><input type="button" id="save" class="btn-blue2 btn-xs" value="保存" /></p></fool:tagOpt> <fool:tagOpt optCode="pddAction3"><p><input type="button" id="copy" class="btn-blue2 btn-xs" value="复制" /></p></fool:tagOpt> <p><input type="button" id="print" class="btn-blue2 btn-xs" value="打印" /></p> <p><input type="button" id="refreshForm" class="btn-blue2 btn-xs" value="刷新"/></p> ');
		...
		$("#btnBox").append('<fool:tagOpt optCode="pddAction3"><p><input type="button" id="copy" class="btn-blue2 btn-xs" value="复制" /></p></fool:tagOpt> <p><input type="button" id="print" class="btn-blue2 btn-xs" value="打印" /></p> <p><input type="button" id="refreshForm" class="btn-blue2 btn-xs" value="刷新"/></p><fool:tagOpt optCode="pddAction5"><p><input type="button" id="cancel" class="btn-blue2 btn-xs" value="作废"/></p></fool:tagOpt>');
	}else if('${obj.recordStatus}'==2){
		$("#inWareHouseId").combotree({hasDownArrow:false});
		$("#billDate").datebox('disable');
		$("#form").find("input").attr('disabled','disabled');
		$("#addGoods").attr('disabled','disabled');
		$("#btnBox").append('<fool:tagOpt optCode="pddAction3"><p><input type="button" id="copy" class="btn-blue2 btn-xs" value="复制" /></p></fool:tagOpt><p><input type="button" id="print" class="btn-blue2 btn-xs" value="打印" /></p>');
	}
	$.get("${ctx}/unitController/getLeafUnit",function(data){
		...
		$("#goodsList").datagrid({
			...
			columns:[[
						...
						{field:'quentity',title:'数量',editor:{type:'numberbox',options:{required:true,precision:2}},sortable:true,width:100},
	...
	$('#form').form('enableValidation'); 
		if($('#form').form('validate')){ 
			$('#save').attr("disabled","disabled");
			    $.post('${ctx}/warehouse/pdd/save',obj,function(data){
...
$('#copy').click(function(e) {
	var fid=$("#fid").val();
	$('#addBox').window("refresh","${ctx}/warehouse/pdd/edit?id="+fid+"&mark=1");
});
$('#verify').click(function(e) {
	...
	$.fool.confirm({title:'确认',msg:'确定要审核该记录吗？',fn:function(r){
		 if(r){
			 $.ajax({
					type : 'post',
					url : '${ctx}/purchaseinquiry/pdd/passAudit',
					data : {id : fid},
					dataType : 'json',
					success : function(data) {	
						if(data.returnCode == '0'){
