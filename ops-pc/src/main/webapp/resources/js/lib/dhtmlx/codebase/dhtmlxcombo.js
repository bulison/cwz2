...
function dhtmlXCombo(parentId, formName, width, optionType, tabIndex) {
	...
	this.conf = {
		...
		opts_count_min: 5, // min count of visible items (when near screen edge)  修改为离最下方bottom的距离最少可以显示5个，超过则变为在input上方显示下拉框
		opts_width: null,
		item_h: null,
		...
		while (t != null && t != this) {
			if (typeof(t._optId) != "undefined") {
				...
				if (r) {
					...
					if(!!window.ActiveXObject || "ActiveXObject" in window){ //IE下延迟隐藏下拉框
						setTimeout(function(){
							that._hideList();
						},200);
					}else{//其他浏览器
						that._hideList();//鼠标点击选择后隐藏下拉框
					}
				}
			}
			t = t.parentNode;
	...
	this._doOnListMouseOut = function(e) {
		...
		that.conf.tm_hover = window.setTimeout(function(){
			...
			//修复鼠标从hover的选项移动到下拉滑动条的时候下拉框消失的问题
			if(e.relatedTarget && $(e.relatedTarget).attr("class")?$(e.relatedTarget).attr("class").indexOf("mytoolsBar")==-1:true && $(that.list).find($(e.relatedTarget)).length <= 1 && that.list.className!=e.relatedTarget.className){
	...
	this._doOnBaseMouseDown = function(e) {
		...
		if (t != this.firstChild) {
			...
			window.setTimeout(function(){that.mykey = 1;that.base.firstChild.focus();},1);//增加了标识mykey，判断是否按了下拉按钮，防止base.js封装组件设置focsShow为true时，按钮失效
...
dhtmlXCombo.prototype.getOption = function(value) { // option by value
	...
	for (var q=0; q<this.list.childNodes.length; q++) {
		if (id == null) {
			...
			if(!a)continue; //下拉框选项新增了工具栏后a会出现undefined，所以跳出防止报错
...
dhtmlXCombo.prototype._showList = function(autoHide) {
	...
	var listWidth = Math.max(this.conf.opts_width||this.conf.col_w||0, this.conf.combo_width);
	var windowsWidth = $(window).outerWidth();
	//当输入框接近右边缘的时候，并且下拉框已经超过右边缘，下拉框位置作出相应调整
	this.list.style.left = listWidth+window.dhx4.absLeft(this.base)>windowsWidth?windowsWidth-listWidth+"px":window.dhx4.absLeft(this.base)+"px";
...
dhtmlXCombo.prototype._checkListHeight = function() {
	...
	if($(this.hdr).find(".myAdd").length > 0){//仓储编辑页的列表货品下拉框特殊情况
		onBottom = Math.max(0, Math.floor((s.bottom-(by+bh+hh)-78)/this.conf.item_h));
	}
	if($(this.list).find(".mytoolsBar").length > 0){//加入了工具栏下拉框特殊情况
		onBottom = Math.max(0, Math.floor((s.bottom-(by+bh+hh)-42)/this.conf.item_h));
	}
	...
	if($(this.hdr).find(".myAdd").length > 0){//仓储编辑页的列表货品下拉框特殊情况
		this.list.style.top = (onBottom==null?by-this.list.offsetHeight+ofs-78:by+bh+hh-ofs)+"px";
		if (this.hdr != null && this.conf.template.header == true) this.hdr.style.top = (onBottom==null?by-hh-this.list.offsetHeight+ofs-78:by+bh-ofs)+"px";
	}
	if($(this.list).find(".mytoolsBar").length > 0){//加入了工具栏下拉框特殊情况
		this.list.style.top = (onBottom==null?by-this.list.offsetHeight+ofs-42:by+bh+hh-ofs)+"px";
		if (this.hdr != null && this.conf.template.header == true) this.hdr.style.top = (onBottom==null?by-hh-this.list.offsetHeight+ofs-42:by+bh-ofs)+"px";
	}
};
...
dhtmlXCombo.prototype._confirmSelect = function(mode, hideList) {
	...
	if(this.base.firstChild == null){return;}//修复IE下不存在firstChild的时候报错
	if (this.conf.last_hover != null) {
		// select value
...
dhtmlXCombo.prototype._mcMakeTemplate = function(cols) {
	...
	for (var q=0; q<cols.length; q++) {
		...
		t += "<div class='dhxcombo_cell"+cssIE+" "+css+"' style='width:"+w+"px;'><div class='dhxcombo_cell_text' title='"+(cols[q].option||"&nbsp;")+"'>"+(cols[q].option||"&nbsp;")+"</div></div>";
...
dhtmlXCombo.prototype.modes.option = {
	...
	render: function(item, data) {
		...
		if(typeof data.text.child != "undefined"){
			$(item).find(".dhxcombo_option_text").addClass("dhx_childOption"+data.text.child);
		}
		if(typeof data.text.mypar != "undefined" && data.text.mypar == 1){
			$(item).find(".dhxcombo_option_text").addClass("dhx_parOption");
		}
	...
	getText: function(item, asStringInput, asStringOption) {
		if (window.dhx4.s2b(asStringInput) && typeof(item._conf.text) == "object") return window.dhx4.template(item._tpl.input, item._conf.text, true).replace(/&nbsp;/, "");
	...
	replaceHtml: function(text) {
		...
		} else if(typeof(text) == "string"){//判断是否字符串，解决了之前replace报错的问题
		...
		}else if(typeof(text) == "number"){//判断是数字则返回数字
			return text;
		}
		return t;
	}
