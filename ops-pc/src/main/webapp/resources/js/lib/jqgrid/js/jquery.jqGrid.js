	...
	return this.each( function() {
			...
			if(ts.p.pginput===true) {
				$("#"+pgcnt).on('keypress','input.ui-pg-input', function(e) {
					...
					if(key === 13) {
						...
						ts.p.page = ($(this).val()>0) ? ($(this).val()>=ts.p.lastpage?ts.p.lastpage:$(this).val()):1;
						populate();
						return false;
					}
					return this;
		...
		$(ts).before(grid.hDiv).click(function(e) {
			...
			if (ts.grid.prevRowHeight && ts.p.scroll && opts.page === undefined) {
				...
				setTimeout(function(){ts.grid.populate();},100);//修复输入大于或等于最大页数时数据不正确的问题
	            setTimeout(function(){ //延迟判断页数是否大于最大页数或者小于等于0，是的话重新加载为最后一页或者第一页
	            	if(ts.p.page > ts.p.lastpage && ts.p.lastpage!=0){
	...
	getRowData : function( rowid, usedata ) {
		...
		this.each(function(){
			...
			if(rowid == null) {
				getall = true;
				resall = [];
				len = $t.rows.length;//修复每次获取整个列表的数据时总是获取少一行的问题
			} else {
