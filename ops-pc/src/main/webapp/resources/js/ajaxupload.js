...
Ajax_upload = AjaxUpload = function(button, options,className){
	if (button.jquery){
...
AjaxUpload.prototype = {
	 ...
	 */
	_createInput : function(className){
		var self = this;
		...
		if(className){
			...
			input.setAttribute('class', "absFileUpload");
		}
		...
		var styles = {
			...
			,'padding': 0
			,'width': $(self._button).width()+'px'//'220px' 调整宽度
			,'height': '30px'
			,'fontSize': '14px'								
			,'opacity': 0
			,'cursor': 'pointer'
	...
	submit : function(){
		...
		//input
		var size=this._input.files[0].size;

		// execute user event
		if (! (settings.onSubmit.call(this, file, getExt(file),size) == false)) {
