...
//------------------------------系统开发使用-------------------------------//
function createCAObject(){
	var caObj = null;
	var divObj=document.getElementById("div1");
	var len = divObj.childNodes.length;
	if(len == 0){
		caObj=document.createElement("object");
		divObj.appendChild(caObj);
		caObj.classid="CLSID:EE762E71-C7D7-4761-9134-1316AC18E531";
		caObj.id="PkiCom3";
	
		caObj=document.createElement("object");
		divObj.appendChild(caObj);
		caObj.classid="CLSID:0DC9CA0C-ED42-4C87-A335-1B0BD043FC70";
		caObj.id="PSIAtv";
	
		caObj=document.createElement("object");
		divObj.appendChild(caObj);
		caObj.classid="clsid:2B1D668B-A0D5-492F-8430-04E663D095FB";
		caObj.id="GDCACom";
	}
}

function _getKeyId_4linux(CAPlugin){
	//以下获得的是证书介质的KeyID
	var keyID = CAPlugin.GDCA_Base64Decode(CAPlugin.GDCA_ReadLabel("LAB_DISAID",3));
	return keyID;
}

function createCAObject_4linux(){
	var divObj=document.getElementById("div1");	
	divObj.innerHTML="<embed type='application/simple-plugin' id='CAPlugin' name='CAPlugin' width=0 height=0 hidden='true' >";
	divObj.style.display="block";
}

function getDisaKeyID(datasetUser){
	if(browserType != BROWSER_IE){
		return getDisaKeyID_4linux(datasetUser);
	}
	createCAObject();
	var ret;
	var UseCom;
	...
	var PkiCom3 = document.all["PkiCom3"];
	var PSIAtv = document.all["PSIAtv"];
	var GDCACom =  document.all["GDCACom"];
...
//判断签名证书的证书时间的有效性（判断证书是否过期）
   ...
   if(ret != 0)
   {
		alert("读取证书硬件介质号出错:"+ret);
		return false;
   }
   	if(checkCAKey(KeyID[0])=="true"){//checkCAKey方法由业务系统实现
   		alert("CA卡已经绑定用户!");
   		return false;
   	}else{
		datasetUser.setValue("caCard",KeyID[0]);
	}
	
	ActiveXEnd(UseCom,KeyType[0]);
}


function getDisaKeyID_4linux(datasetUser){
	createCAObject_4linux();
	var ret;
   
	var CAPlugin = document.getElementById("CAPlugin");
	if( CAPlugin.GDCA_SetDeviceType(10) != 0){
		alert("设备类别设置出错,请确认您使用的是3.0及以上版本的Key!");
		return false;
	}
	if(CAPlugin.GDCA_Initialize()!=0 ){
		 alert("客户端控件初始化失败。");
		 return false;
	}
	...
	//读取用户Key中的用户证书
	//读取签名证书
   var keyId = _getKeyId_4linux(CAPlugin);
...
//判断签名证书的证书时间的有效性（判断证书是否过期）
   ...
   if(ret != 0)
   {
	...
	if(checkCAKey(keyId)=="true"){//checkCAKey方法由业务系统实现
   		alert("CA卡已经绑定用户!");
   		return false;
   	}else{
		datasetUser.setValue("caCard",keyId);
	}
...
function verys(url){
    ...
    var pin=document.all['caCard'].value;
    //document.forms[0].password.value=pin;
...
//判断签名证书的证书时间的有效性（判断证书是否过期）
   ...
   if(ret != 0)
   {
	...
	document.all["user_pin"].value=pin;
    document.all["sign_cert"].value = CertData[0];
    document.all["enc_cert"].value = CertData2[0];
	document.all["userKeyID"].value = KeyID[0];
	document.all["KeyType"].value = KeyType[0];
...
function verys_4linux(url){
	createCAObject_4linux();
	var ret;
	var CAPlugin = document.all["CAPlugin"];
    var pin=document.all['caCard'].value;
	...
	//ret = GDCALogin(UseCom,KeyType[0],pin);
	if(CAPlugin.GDCA_Login(2,pin) !=0 ){
		_reset();
		alert("客户端控件登陆失败。");
		return false;
	}
	...
	//读取签名证书
   //ret = GDCAReadCert(UseCom,KeyType[0],SignCert,CertData);
   var sign_cert = CAPlugin.GDCA_ReadLabel("LAB_USERCERT_SIG",7);
   if(sign_cert.length>1300){
	   //sign_cert = sign_cert.substring(0,1300);
   }

    //alert(KeyType[0]);	
   //读取加密证书
   //ret = GDCAReadCert(UseCom,KeyType[0],EncCert,CertData2);
   var enc_cert =  CAPlugin.GDCA_ReadLabel("LAB_USERCERT_SIG",8);
/*
//判断签名证书的证书时间的有效性（判断证书是否过期）
   ret=GDCAGetCerTime(UseCom,KeyType[0],CertData[0]);
   if(ret != 0)
   ...
   //ret = GDCAGetKeyID(UseCom,KeyType[0],KeyID,CertData[0] );
	...
	document.all["user_pin"].value = pin;
    document.all["sign_cert"].value = sign_cert;
    document.all["enc_cert"].value = enc_cert;
	document.all["userKeyID"].value = keyId;
	document.all["KeyType"].value = "2";
	document.forms[0].action= url;
	document.forms[0].submit();
	return true;
}
