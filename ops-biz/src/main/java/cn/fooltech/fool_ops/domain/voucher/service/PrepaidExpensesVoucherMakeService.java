 ...
 * <p>待摊费用生成凭证</p>
 * @author lgk
 * @date 2016年4月18日下午03:30:35
 * @version V1.0
 */
@Component
public class PrepaidExpensesVoucherMakeService extends VoucherMakeAbstractService{
	 ...
	 */
	@Autowired
	private PrepaidExpensesDetailService prepaidDetailService;
	...
	public RequestResult makeVoucher(VoucherMakeVo vo) {
		...
		for(String billId : billIds){
			PrepaidExpensesDetail prepaidDetail = prepaidDetailService.get(billId);
			PrepaidExpenses prepaidExpenses = prepaidDetail.getExpenses();
			Organization dept = prepaidExpenses.getDept();
			
			//如果单据之前已经生成过凭证，则不能再次生成
			Voucher record = voucherBillService.getVoucher(prepaidDetail.getFid(), WarehouseBuilderCodeHelper.dtfy, fiscalAccount.getFid());
			if(record != null){
				continue;
			}
			
			//借方科目
			FiscalAccountingSubject debitSubject = prepaidExpenses.getDebitSubject();
			if(debitSubject == null){
				if(isOne){
					return new RequestResult(RequestResult.RETURN_FAILURE, "借方科目不能为空!");
				}
				else{
					totalFail++;
					continue;
			...
			//贷方科目
			FiscalAccountingSubject creditSubject = prepaidExpenses.getCreditSubject();
			if(creditSubject == null){
				if(isOne){
					return new RequestResult(RequestResult.RETURN_FAILURE, "贷方科目不能为空!");
				}
				else{
					totalFail++;
					continue;
			...
			RequestResult debitCheck = check(null, debitSubject, dept);
			...
			RequestResult creditCheck = check(null, creditSubject, dept);
			...
			//添加凭证明细
			addVoucherDetail(voucher, prepaidExpenses, prepaidDetail, fiscalAccount, getSubject(debitSubject, dept), FiscalAccountingSubject.DIRECTION_BORROW, vo.getVoucherResume());
			addVoucherDetail(voucher, prepaidExpenses, prepaidDetail, fiscalAccount, getSubject(creditSubject, dept), FiscalAccountingSubject.DIRECTION_LOAN, vo.getVoucherResume());
			
			//添加单据、凭证关联
			VoucherBill voucherBill = new VoucherBill();
			voucherBill.setVoucher(voucher);
			voucherBill.setBillType(WarehouseBuilderCodeHelper.dtfy);
			voucherBill.setBillId(prepaidDetail.getFid());
	 ...
	 * @return
	 */
	private void addVoucherDetail(Voucher voucher, PrepaidExpenses prepaidExpenses, PrepaidExpensesDetail detail, FiscalAccount fiscalAccount, 
			FiscalAccountingSubject subject, Integer direction, String voucherResume){
		...
		if(direction == FiscalAccountingSubject.DIRECTION_BORROW){
			voucherDetail.setDebitAmount(detail.getAmount());
		}
		else{
			voucherDetail.setCreditAmount(detail.getAmount());
		}
		//核算部门
		if(subject.getDepartmentSign() == FiscalAccountingSubject.ACCOUNT){
			voucherDetail.setDepartment(prepaidExpenses.getDept());
		}
		//核算数量
		...
		if(subject.getCurrencySign() == FiscalAccountingSubject.ACCOUNT){
			...
			voucherDetail.setCurrencyAmount(detail.getAmount());
		}
	...
	public RequestResult check(Integer billType, FiscalAccountingSubject subject, Object... args) {
		...
		if(subject.getMemberSign() == FiscalAccountingSubject.ACCOUNT){
			return new RequestResult(RequestResult.RETURN_FAILURE, subject.getName() + "科目设置了核算人员!");
		}
		if(subject.getCurrencySign() == FiscalAccountingSubject.ACCOUNT){
			return new RequestResult(RequestResult.RETURN_FAILURE, subject.getName() + "科目设置了核算币别!");
		}
		return new RequestResult();
	}
