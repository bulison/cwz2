package cn.fooltech.fool_ops.domain.initialReceivable.repository;
...
public interface InitialReceivableRepository extends JpaRepository<WarehouseBill, String>,JpaSpecificationExecutor<WarehouseBill> {
	 ...
	 * @param vo
	 */
	public default Page<WarehouseBill> query(InitialReceivableVo vo,Pageable pageable){
		Page<WarehouseBill> page = findAll(new Specification<WarehouseBill>() {
			...
			public Predicate toPredicate(Root<WarehouseBill> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
				...
				predicates.add(builder.equal(root.get("billType"), WarehouseBuilderCodeHelper.qcys));
