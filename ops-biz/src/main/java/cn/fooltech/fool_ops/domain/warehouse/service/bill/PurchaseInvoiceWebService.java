 ...
 * @date 2015年9月24日
 */
@Service("ops.PurchaseInvoiceWebService")
public class PurchaseInvoiceWebService extends BaseWarehouseWebServiceBuilder{
	
	/**
	 * 单据关联服务类
	 */
	@Autowired
	private BillRelationService relationService;
	
	@Override
	@Value("#{T(cn.fooltech.fool_ops.domain.warehouse.util.WarehouseBuilderCode).cgfp}")
	...
	public RequestResult save(WarehouseBillVo vo) {
		...
		if(parentResult.getReturnCode()==RequestResult.RETURN_SUCCESS && 
			...
			int refType = WarehouseBuilderCodeHelper.getBillType(WarehouseBuilderCode.cgrk);
