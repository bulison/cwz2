 ...
 */
@Component("ops.ShdBillStrategy")
public class ShdBillStrategy implements BillStrategy {

    /*
    审核：
    期间总库存金额表:
    本期入库数量 = 本期入库数量 + 单据实收数量；（取消）
    本期入库金额 = 本期入库金额 + 单据成本金额；（取消）
    本期结存数量 = 本期结存数量 + 单据实收数量；
...
//            new CalMethod("inAmount", "inAmount", CalMethod.Compute.Add, "accountQuentity", CalMethod.Compute.Mul, "accountPrice"),
            ...
            new CalMethod("transportInQuantity", "transportInQuantity", CalMethod.Compute.Add, "accountQuentity"),
            new CalMethod("transportInAmount", "transportInAmount", CalMethod.Compute.Add, "costAmount")
...
//            new CalMethod("inAmount", "inAmount", CalMethod.Compute.Sub, "accountQuentity", CalMethod.Compute.Mul, "accountPrice"),
            ...
            new CalMethod("transportInQuantity", "transportInQuantity", CalMethod.Compute.Sub, "accountQuentity"),
            new CalMethod("transportInAmount", "transportInAmount", CalMethod.Compute.Sub, "costAmount")
...
//            new CalMethod("inAmount", "inAmount", CalMethod.Compute.Sub, "accountQuentity", CalMethod.Compute.Mul, "accountPrice"),
    ...
    */
    ...
    public boolean isSupport(int billType) {
        return billType == WarehouseBuilderCodeHelper.shd;
    }
