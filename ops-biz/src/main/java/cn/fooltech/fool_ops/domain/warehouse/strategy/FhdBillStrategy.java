 ...
 */
@Component("ops.FhdBillStrategy")
public class FhdBillStrategy implements BillStrategy {

    /*
    审核：
    期间总库存金额表:
    本期出库数量 = 本期出库数量 + 单据记账数量；（取消）
    本期出库金额 = 本期出库金额 + 单据数量 * 单据成本单价；（取消）
    本期结存数量 = 本期结存数量 - 单据数量；
...
//            new CalMethod("outAmount", "outAmount", CalMethod.Compute.Add, "accountQuentity", CalMethod.Compute.Mul, "accountPrice"),
            ...
            new CalMethod("transportOutQuantity", "transportOutQuantity", CalMethod.Compute.Add, "accountQuentity"),
            new CalMethod("transportOutAmount", "transportOutAmount", CalMethod.Compute.Add, "costAmount")
...
//            new CalMethod("outAmount", "outAmount", CalMethod.Compute.Sub, "accountQuentity", CalMethod.Compute.Mul, "accountPrice"),
            ...
            new CalMethod("transportOutQuantity", "transportOutQuantity", CalMethod.Compute.Sub, "accountQuentity"),
            new CalMethod("transportOutAmount", "transportOutAmount", CalMethod.Compute.Sub, "costAmount")
...
//            new CalMethod("outAmount", "outAmount", CalMethod.Compute.Sub, "accountQuentity", CalMethod.Compute.Mul, "accountPrice"),
    ...
    */
    ...
    public boolean isSupport(int billType) {
        return billType == WarehouseBuilderCodeHelper.fhd;
    }
